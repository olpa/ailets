...
async fn test_different_std_handles() {
    ...
    pool.touch_writer(actor_handle, StdHandle::Stdout as isize, &id_gen)
        .await
        .expect("Failed to create stdout pipe");

    pool.touch_writer(actor_handle, StdHandle::Log as isize, &id_gen)
        .await
...
async fn test_reader_on_latent_pipe_closed_without_realizing() {
    ...
    pool.flush_close_actor_writers(actor_handle, 0).await.unwrap();
...
async fn test_write_to_realized_writer() {
    ...
    // Get writer
    let writer = pool.get_already_realized_writer((actor_handle, std_handle as isize));
...
async fn test_concurrent_writers_for_different_handles() {
    ...
    for i in 1..=5 {
        ...
        let task = tokio::spawn(async move {
            ...
            pool_clone
                .touch_writer(actor_handle, StdHandle::Stdout as isize, &id_gen_clone)
                .await
        });
        tasks.push(task);
    }
    ...
    for i in 1..=5 {
        assert!(pool
            .get_already_realized_writer((Handle::new(i), StdHandle::Stdout as isize))
            .is_some());
    }
}
...
async fn test_create_output_pipe_allocates_buffer() {
    ...
    let buffer_name = pipe_path(actor_handle, std_handle as isize);
    let buffer = kv
...
async fn test_realize_already_realized_is_noop() {
    ...
    let result = pool.touch_writer(actor_handle, std_handle as isize, &id_gen).await;
...
async fn test_mixed_latent_and_realized_pipes() {
    ...
    pool.touch_writer(Handle::new(1), StdHandle::Stdout as isize, &id_gen)
        .await
    ...
    let _reader_task = tokio::spawn(async move {
        pool_clone
            .get_or_await_reader((Handle::new(2), StdHandle::Stdout as isize), true, &id_gen_clone)
            .await
    });
    ...
    let writer = pool.get_already_realized_writer((Handle::new(2), StdHandle::Stdout as isize));
...
async fn test_dependency_reading_simulation() {
    ...
    let writer = pool
        .get_already_realized_writer((actor1, StdHandle::Stdout as isize))
        .unwrap();
...
async fn test_race_touch_writer_vs_close() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            pool_clone
                .touch_writer(actor_handle, std_handle as isize, &id_gen_clone)
                .await
        });
        handles.push(handle);
    }
    ...
    let close_handle = tokio::spawn(async move {
        ...
        pool_clone.flush_close_actor_writers(actor_handle, 0).await.unwrap();
    });
...
async fn test_close_actor_writers_with_error_reader_sees_epipe() {
    ...
    pool.flush_close_actor_writers(actor_handle, EOWNERDEAD).await.unwrap();
