...
async fn test_empty_write_on_closed_writer() {
    ...
    // Empty write on closed writer should return error (EBADF)
...
async fn test_empty_write_with_errno() {
    ...
    // Empty write should return error, not 0
    let result = writer.write(b"");
    assert!(result.is_err());
...
async fn test_reader_dont_read_when_error() {
    ...
    // Try to read - should return error without reading data
    let mut buf = [0u8; 10];
...
async fn test_reader_read_with_writer_error() {
    ...
    // Next read should return error with EPIPE, not the writer's raw errno
    let result = reader.read(&mut buf).await;
    assert!(result.is_err());
...
async fn test_reader_own_error_takes_precedence() {
    ...
    // read() should return error with reader's error
    let mut buf = [0u8; 10];
    let result = reader.read(&mut buf).await;
    assert!(result.is_err());
...
async fn test_multiple_readers_independent_errors() {
    ...
    // Both return error on read with their own errors
    let mut buf = [0u8; 10];
