...
fn output_tool_call() {
    ...
    let mut builder = StructureBuilder::new(writer.clone());
    let tool_call = ContentItemFunction::new("call_123", "get_user_name", "{}");
...
fn streaming_arguments_chunking() {
    ...
    builder
        .stream_tool_call_arguments_chunk(&mut rjiter)
        .unwrap();
...
fn streaming_arguments_multiple_chunks() {
    ...
    builder
        .stream_tool_call_arguments_chunk(&mut rjiter1)
        .unwrap();
    ...
    builder
        .stream_tool_call_arguments_chunk(&mut rjiter2)
        .unwrap();
